...
public class DiscountCalculatorServiceTest {
	...
	public void processExcelFileTest() throws IOException {
		 ...
		 * 
		 * final Long lastModified = 0L; final List<Product> products = new
		 * ArrayList<Product>(); final Product product = new Product();
		 * product.setProductId("AB101"); product.setListPrice(new BigDecimal(101));
		 * final List<Discount> discounts = new ArrayList<Discount>();
		 * 
		 * final Discount discount = new Discount(); discount.setDiscountId(1L);
		 ...
		 * discounts.add(discount); product.setDiscounts(discounts);
		 ...
		 * 
		 * when(processedFilesRepository.findByFileNameAndProcessedDate(any(),
		 * any())).thenReturn(null); when(processedFilesRepository.save(new
		 * ProcessedFiles())).thenReturn(new ProcessedFiles());
		 * 
		 * service.processExcelFile(FILE_NAME, lastModified);
		 * 
		 * verify(productRepository, times(1)).findOne(anyString());
		 * verify(productRepository, times(2)).save(products); verify(productRepository,
		 * times(1)).findAll(); verify(processedFilesRepository,
		 * times(1)).findByFileNameAndProcessedDate(any(), any());
		 */}
